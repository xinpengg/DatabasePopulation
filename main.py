...
for department, courses in departments.items():
    if department not in inserted_departments:
        ...
        for course in courses:
            ...
            major_assignment_start = assignment_id  # Set the start point for major assignments
            for i in range(major_assignment_start, major_assignment_start + num_major_assignments):
                print(f"INSERT INTO Assignments (assignment_id, name, assignment_type, course_id) "
                      f"VALUES ({assignment_id}, 'Major Assignment {assignment_id - major_assignment_start + 1}', 1, {num_courses});")
                assignment_id += 1  # Increment assignment_id
